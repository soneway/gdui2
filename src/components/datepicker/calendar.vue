<template>
  <div
    class="gd-datepicker-calendar"
    :class="'gd-datepicker-calendar-panel-' + panel.toLowerCase()">
    <div class="gd-datepicker-calendar-header">
      <a
        v-show="panel !== 'TIME'"
        class="gd-datepicker-icon-last-year"
        @click="handleIconYear(-1)">&#171;</a>
      <a
        v-show="panel === 'DATE'"
        class="gd-datepicker-icon-last-month"
        @click="handleIconMonth(-1)">&#8249;</a>
      <a
        v-show="panel !== 'TIME'"
        class="gd-datepicker-icon-next-year"
        @click="handleIconYear(1)">&#187;</a>
      <a
        v-show="panel === 'DATE'"
        class="gd-datepicker-icon-next-month"
        @click="handleIconMonth(1)">&#8250;</a>
      <a
        v-show="panel === 'DATE'"
        class="gd-datepicker-current-month"
        @click="handleBtnMonth">{{ months[calendarMonth] }}</a>
      <a
        v-show="panel === 'DATE' || panel === 'MONTH'"
        class="gd-datepicker-current-year"
        @click="handleBtnYear">{{ calendarYear }}</a>
      <a
        v-show="panel === 'YEAR'"
        class="gd-datepicker-current-year">{{ yearHeader }}</a>
      <a
        v-show="panel === 'TIME'"
        class="gd-datepicker-time-header"
        @click="handleTimeHeader">{{ timeHeader }}</a>
    </div>
    <div class="gd-datepicker-calendar-content">
      <PanelDate
        v-show="panel === 'DATE'"
        :value="value"
  ...
  export default {
    ...
    methods: {
      handelPanelChange (panel, oldPanel) {
        this.dispatch('gd-datepicker', 'panel-change', [panel, oldPanel]);
        if (panel === 'YEAR') {
          this.firstYear = Math.floor(this.calendarYear / 10) * 10;
        }
        else if (panel === 'TIME') {
          this.$nextTick(() => {
            const list = this.$el.querySelectorAll('.gd-datepicker-panel-time .gd-datepicker-time-list');
      ...
      updateNow (value) {
        ...
        if (this.visible && this.now !== oldNow) {
          this.dispatch('gd-datepicker', 'calendar-change', [new Date(this.now), new Date(oldNow)]);
        }
      },
      ...
      selectYear (year) {
        ...
        this.dispatch('gd-datepicker', 'select-year', [year, this.index]);
        this.showPanelMonth();
      },
      selectMonth (month) {
        ...
        this.dispatch('gd-datepicker', 'select-month', [month, this.index]);
        this.showPanelDate();
      },
      selectTime (time) {
