...
export default {
  ...
  render (h) {
    const { calendarYear, calendarMonth, firstDayOfWeek } = this;
    const ths = this.getDays(firstDayOfWeek)
      .map(day => <th>{day}</th>);

    const dates = this.getDates(calendarYear, calendarMonth, firstDayOfWeek);
    const tbody = Array.apply(null, { length: 6 })
      .map((week, i) => {
        const tds = dates.slice(7 * i, 7 * i + 7)
          .map(date => <td
            class={['cell'].concat(this.getCellClasses(date))}
    ...
    return (
      <table class="gd-datepicker-panel gd-datepicker-panel-date">
        <thead>
          <tr>{ths}</tr>
        </thead>
        <tbody>
