...
export default {
  ...
  render (h) {
    ...
    if (Array.isArray(pickers) && pickers.length) {
      pickers = pickers.map(picker => {
        ...
        return (
          <li
            class={{
              'gd-datepicker-time-picker-item': true,
              cell: true,
              actived:
      ...
      return (
        <div class="gd-datepicker-panel gd-datepicker-panel-time">
          <ul class="gd-datepicker-time-list">{pickers}</ul>
        </div>
      );
    }
    ...
    times = times.map(list => (
      <ul
        class="gd-datepicker-time-list"
        style={{ width: 100 / times.length + '%' }}>
        {list}
      </ul>
    ));

    return <div class="gd-datepicker-panel gd-datepicker-panel-time">{times}</div>;
  },
};
