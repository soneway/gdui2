<template>
  <div
    ...
    <div
      class="gd-datepicker-input-wrapper"
      @click="showPopup">
      <input
        :class="inputClass"
        :name="inputName"
      ...
      <span
        v-if="showClearIcon"
        class="gd-datepicker-input-append gd-datepicker-clear-wrapper"
        @click.stop="clearDate">
        <slot name="gd-datepicker-clear-icon">
          <i class="gd-datepicker-input-icon gd-datepicker-clear-icon"></i>
        </slot>
      </span>
      <span class="gd-datepicker-input-append">
        <slot name="calendar-icon">
          <svg
            xmlns="http://www.w3.org/2000/svg"
            version="1.1"
            viewBox="0 0 200 200"
            class="gd-datepicker-calendar-icon">
            <rect
              x="13"
              y="29"
              rx="14"
            ...
            <text
              ...
              stroke-width="1"
              text-anchor="middle"
              dominant-baseline="middle">{{ new Date().getDate() }}</text>
          </svg>
        </slot>
      </span>
    </div>
    <div
      class="gd-datepicker-popup"
      ...
      <slot name="header">
        <div
          class="gd-datepicker-shortcuts-wrapper"
          v-if="range && innerShortcuts.length">
          <button
            type="button"
            class="gd-datepicker-shortcuts"
            v-for="(range, index) in innerShortcuts"
            :key="index"
            @click="selectRange(range)">{{ range.text }}
          </button>
        </div>
      </slot>
      <CalendarPanel
      ...
      <div
        class="gd-datepicker-range-wrapper"
        v-else>
        <CalendarPanel
          ...
          @select-time="selectStartTime"></CalendarPanel>
        <CalendarPanel
          v-bind="$attrs"
          :index="1"
          :type="innerType"
          ...
          @select-time="selectEndTime"></CalendarPanel>
      </div>
      <slot
        name="footer"
        :confirm="confirmDate">
        <div
          class="gd-datepicker-footer"
          v-if="confirm">
          <button
            type="button"
            class="gd-datepicker-btn gd-datepicker-btn-confirm"
            @click="confirmDate">{{ confirmText }}
          </button>
        </div>
      </slot>
    </div>
...
<script>
  ...
  export default {
    ...
    computed: {
      ...
      showClearIcon () {
        return !this.disabled && !this.readonly && this.clearable && (this.range ? this.isValidRangeValue(this.value) : this.isValidValue(this.value));
      },
      innerType () {
    ...
    watch: {
      ...
      currentValue (value) {
        this.fixText = this.getText(value);
      },
    },
    ...
    methods: {
      getText (value) {
        ...
        return `${this.stringify(value[0])} ${this.rangeSeparator} ${this.stringify(value[1])}`;
      },
      initCalendar () {
      ...
      handleChange () {
        if (!this.readonly && this.userInput !== null) {
